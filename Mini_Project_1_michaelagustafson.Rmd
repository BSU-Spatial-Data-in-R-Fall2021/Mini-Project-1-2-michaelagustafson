...
} #I can't run this to replicate your nz pharm

#write.csv(nz.pharm, here("nz.pharm.csv"))

# read in pharmacy csv created above that now has location data:

nz.pharm <- read.csv(here("nz.pharm.csv"))# this doesn't work and your document doesn't knit; presumably because you didn't save or upload the nz.pharm object
...
### Create buffers around attack points and calculate mean elevation
...
elev.at.attack$ID <- zombie.attacks.df$OBJECTID
...
>>>>>>> ffc0b39210682529acf224f94b2677f3794919e4
...
```{r}
tidy.summary.df <- st_as_sf(tidy.summary.df)
?tm_polygons

tm_shape(tidy.summary.df) +
...
tm_shape(tidy.summary.df) + 
  tm_polygons(col = "total_attacks", palette = "magma") +
tm_shape(nz.pharm.proj) +
  tm_dots("type", col = "green", size = 0.1)


```
Hmmm... I think these colors are too similar... Also, plotting the map this way already has me thinking of other ways I would like to have my data setup in the dataframe to get across the message I'm trying to put out.


```{r}
st_bbox(nz.full.shp)
bg <- get_stamenmap(as.vector(st_bbox(nz.full.shp, 4326)), zoom = 18)

ggmap(bg) +
  geom_sf(data = nz.pharm.proj)
```
